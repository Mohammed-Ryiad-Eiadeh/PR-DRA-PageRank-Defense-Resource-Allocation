...
public class PageRank_PR_DRA {
     ...
     */
    public PageRank_PR_DRA(double[][] adjMatrix, double[] lossVector, int maxIteration, PR_DenominatorType type) {
        if (adjMatrix == null) {
        ...
        for (int iter = 0; iter < maxIter; iter++) {
            for (int nod : mapNodeToRank.keySet()) {
                ...
                // If the parents of the given nod have parents (in degree links) then do the following
                    ...
                    // To run the optimized version, please uncomment these two terms: '+ 0.001 * assetLossVec[nod]'
                    totalRank = (1 - dampingFactor) / numberOfNodes + dampingFactor * totalRank /*+ 0.001 * assetLossVec[nod]*/;
            ...
            // Take a copy of the ranks and apply Euclidean distance to check the convergence of PageRank_PR_DRA given the epsilon
            if (iter == 1) {
